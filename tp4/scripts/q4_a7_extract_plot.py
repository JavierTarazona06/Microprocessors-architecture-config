...
os.environ.setdefault("MPLCONFIGDIR", "/tmp/mplconfig_gem5_q4")
import matplotlib
...
def create_plots(rows, figures_dir: Path):
    ...
    for app in APPS:
        ...
        _plot_common(
            figures_dir / f"q4_a7_{app}_ipc_vs_l1.png",
            f"A7 {app}: IPC vs L1",
            "L1 size (KB)",
            "IPC",
        )
        ...
        _plot_common(
            figures_dir / f"q4_a7_{app}_cycles_vs_l1.png",
            f"A7 {app}: numCycles vs L1",
            "L1 size (KB)",
            "numCycles",
        )
        ...
        plt.legend()
        _plot_common(
            figures_dir / f"q4_a7_{app}_miss_rates_vs_l1.png",
            f"A7 {app}: miss rates vs L1",
            "L1 size (KB)",
            "Miss rate (%)",
        )
        ...
        _plot_common(
            figures_dir / f"q4_a7_{app}_branch_mispred_vs_l1.png",
            f"A7 {app}: branch misprediction rate vs L1",
            "L1 size (KB)",
            "Misprediction rate (%)",
        )
...
def write_run_params(path: Path, runs_base: Path):
    content = f"""Q4 A7 gem5 run parameters
- CPU model: DerivO3CPU (out-of-order)
- Config script: se_A7.py
- L2 cache: 512kB, assoc=8, block=32B (fixed)
- L1 sweep: I-L1 = D-L1 = 1kB, 2kB, 4kB, 8kB, 16kB
- Branch predictor: BiModeBP/BranchPredictor wrapper with BTB=256
- Decode/Issue/Commit: 2/4/2
- RUU/LSQ (gem5 mapping): ROB=2, LQ=8, SQ=8
- Fetch queue: 8
...
def parse_args():
    parser = argparse.ArgumentParser(description="Extract Q4 A7 metrics and generate plots.")
    parser.add_argument("--runs-base", required=True, help="Base directory containing A7 run folders")
